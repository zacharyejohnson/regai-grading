...
const prepareScorm = (assignmentId, assignmentData) => {
  const rootDir = path.join(__dirname, '..');
  const buildDir = path.join(rootDir, 'build');
  const publicDir = path.join(rootDir, 'public');
  const scormDir = path.join(rootDir, `scorm-package-${assignmentId}`);
  ...
  // Update index.html to use relative paths and include the assignment data
